...
/* CronoGol v2.6.5 - Online Lobby Visual Enhancer */
(function(){
"use strict";
function text(el){return String(el&&el.textContent||"").trim().toLowerCase()}
function enhanceOnlineLobby(){
 const nodes=Array.from(document.querySelectorAll("section,div,article,main"));
 const lobby=nodes.find(el=>{const t=text(el);return t.includes("lobby de sala privada")||(t.includes("crear sala")&&t.includes("unirse")&&t.includes("código"))});
 if(!lobby)return false;
 lobby.classList.add("online-lobby");
 const title=Array.from(lobby.querySelectorAll("h1,h2,h3,p,span,div")).find(el=>text(el).includes("lobby de sala privada"));
 if(title&&!title.querySelector(".online-lobby-badge")){title.classList.add("online-lobby-header");title.textContent="";const b=document.createElement("span");b.className="online-lobby-badge";b.textContent="V2 ONLINE";const l=document.createElement("span");l.textContent="LOBBY DE SALA PRIVADA";title.appendChild(b);title.appendChild(l)}
 const code=Array.from(lobby.querySelectorAll("*")).find(el=>/^[A-Z0-9]{5,8}$/.test(String(el.textContent||"").trim())); if(code){code.classList.add("online-room-code"); if(code.parentElement)code.parentElement.classList.add("online-code-row")}
 Array.from(lobby.querySelectorAll("button")).forEach(btn=>{const t=text(btn);if(t.includes("crear sala"))btn.id=btn.id||"onlineCreateRoomBtn";if(t.includes("actualizar"))btn.id=btn.id||"onlineRefreshRoomBtn";if(t.includes("unirse"))btn.id=btn.id||"joinRoomBtn";if(t.includes("limpiar"))btn.classList.add("online-clear-room")});
 Array.from(lobby.querySelectorAll("input")).forEach(inp=>{if((inp.placeholder||"").toLowerCase().includes("código")||(inp.placeholder||"").toLowerCase().includes("sala"))inp.classList.add("online-join-input")});
 Array.from(lobby.querySelectorAll("*")).forEach(el=>{const t=text(el);if(t.includes("conectado"))el.classList.add("online-status-pill","connected");if(t.includes("esperando rival"))el.classList.add("online-status-pill","waiting");if(t.includes("sala ")&&t.includes("creada"))el.classList.add("online-alert")});
 window.__cronogolOnlineLobbyVisualMatch={version:"2.6.5",applied:true,at:new Date().toISOString()};
 return true;
}
function run(){enhanceOnlineLobby()}
if(document.readyState==="loading")document.addEventListener("DOMContentLoaded",run);else run();
setInterval(enhanceOnlineLobby,1000);
})();
