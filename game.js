...
function currentRun(){return state&&state.runningState&&state.runningState.isRunning?state.runningState:null} function runElapsed(rs){return Number(rs.baseMs||0)+Math.max(0,serverNow()-Number(rs.startedAtMs||serverNow()))}
...
function applyState(next){if(!next)return;state=next;if(Number.isFinite(Number(next.currentPlayerIndex)))gameState.currentPlayerIndex=Number(next.currentPlayerIndex);if(Array.isArray(next.players)&&Array.isArray(gameState.players))next.players.slice(0,2).forEach((p,i)=>{if(!gameState.players[i])return;if(typeof p.name==="string")gameState.players[i].name=p.name.replace(/[\u0000-\u001F\u007F]/g,"").slice(0,24)||gameState.players[i].name;if(Number.isFinite(Number(p.goals)))gameState.players[i].goals=Number(p.goals);if(Number.isFinite(Number(p.skipTurns)))gameState.players[i].skipTurns=Number(p.skipTurns)});try{updateUI()}catch(e){}if(currentRun())ensureRaf();else clearRaf();if(next.lastStoppedThrow&&next.lastStoppedThrow.stopId!==lastStopId){lastStopId=next.lastStoppedThrow.stopId;paintStop(next.lastStoppedThrow)}controls()}
