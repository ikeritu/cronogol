...
/* ===== CronoGol v2.5.7: support bugfix + clean share/support functions ===== */
...
/* ===== CronoGol v2.5.7: game feel improvements ===== */
...
/* ===== CronoGol v2.5.7: Project Audit Fixes =====
...
/* ===== CronoGol v2.5.7: Fast Rules & Stats Polish =====
...
/*
===============================================================================
CronoGol v2.5.7 — Online Turn Control
===============================================================================
...
(function(){
  ...
  const ONLINE_TURN_VERSION = "2.5.7";
...
/* CronoGol v2.5.7 — Online Throws & Score Sync */
(function(){
"use strict";
const V="2.5.7";
...
function ev(kind,v,actor){
 ...
 const payload={id,kind,value:Number.isFinite(Number(v))?Number(v):null,valueText,displayTime:(timerDisplay?timerDisplay.textContent:""),elapsedMs:(Number.isFinite(Number(currentElapsedMs))?Math.round(Number(currentElapsedMs)):null),actorIndex:actor.idx,actorName,title,message:cleanMessage,eventClass:cls(),createdAt:new Date().toISOString()};try{window.__cronogolLastThrowForAudit=payload;}catch(e){}return payload;
}
...
/* CronoGol v2.5.7 — Last Throw Message Fix */
(function(){
  "use strict";
  let lastClean = null;
...
/* CronoGol v2.5.7 — Rules Selector & Online Sanctions Fix */
(function(){
"use strict";
const V="2.5.7",URL="https://xbrrdkflztxkvnngmdhu.supabase.co",KEY="sb_publishable_Ktw6Eh91X5K0yRjA9qJ6VA_vhxLPu8l",TABLE="cronogol_rooms";
let busy=false;
function st(){try{return window.CronoGolOnline&&window.CronoGolOnline.getOnlineStatus?window.CronoGolOnline.getOnlineStatus():{};}catch(e){return {};}}
function code(){return String(st().currentRoomCode||"").trim().toUpperCase();}
function role(){return String(st().currentRole||"").trim().toLowerCase();}
...
/*
===============================================================================
CronoGol v2.5.7 — Menu Final Fix & Host Mode Sync
===============================================================================
Corrección real, no cosmética:
- Reglas queda antes que Rival en HTML.
- Dificultad desaparece visualmente y queda fija en normal.
- Reglas y Rival empiezan sin selección.
- Rival queda bloqueado hasta elegir Clásico o Rápido.
- No se puede empezar sin Reglas + Rival.
- Online: el invitado hereda matchMode del anfitrión.
===============================================================================
*/
(function(){
"use strict";
...
function forceNormalDifficulty(){
  const sel=id("machine-level") || id("difficulty") || id("machine-difficulty") || q('[name="difficulty"]') || q('[name="machine-difficulty"]');
  if(sel) sel.value="normal";
  if(gameState) gameState.machineLevel="normal";
}
function hideDifficulty(){
  try{
    qa('#machine-level-label,#difficulty,#machine-difficulty,[name="difficulty"],[name="machine-difficulty"],.machine-level-field').forEach(el=>{
    ...
    qa("label,.form-label,.field-label,.setting-label").forEach(label=>{
      const t=(label.textContent||"").trim().toLowerCase();
      if(t==="dificultad" || t==="difficulty") label.classList.add("cg-hidden-difficulty");
    });
  }catch(e){}
  forceNormalDifficulty();
}
function setRivalEnabled(enabled){
  qa('.segment-btn[data-target="game-mode"],select#game-mode,[data-target="game-mode"]').forEach(el=>{
    if(enabled){
      el.classList.remove("rules-gated-disabled");
      el.removeAttribute("aria-disabled");
      if(el.tagName==="SELECT") el.disabled=false;
    }else{
      el.classList.add("rules-gated-disabled");
...
function ensureHint(){
  ...
  if(!h){
    ...
    const parent=rivalLabel && rivalLabel.parentElement ? rivalLabel.parentElement : (q(".setup-card")||document.body);
...
function refresh(){
  ...
  rulesChosen=Boolean(rulesChosen || r);
  rivalChosen=Boolean(rivalChosen || gm);

  setRivalEnabled(Boolean(rulesChosen && r));
  setStartEnabled(Boolean(rulesChosen && rivalChosen && r && gm));
...
function markRival(){
  if(clearing) return;
  if(!rulesChosen && home()){ refresh(); return; }
  rivalChosen=true;
  apply();
  refresh();
}
document.addEventListener("click", function(e){
  ...
  if(gb){
    ...
    setTimeout(markRival,0);
  }
}, true);
document.addEventListener("change", function(e){
  if(e.target && e.target.id==="match-mode") setTimeout(markRules,0);
...
function onlineStatus(){
  try{
    return window.CronoGolOnline && window.CronoGolOnline.getOnlineStatus ? window.CronoGolOnline.getOnlineStatus() : {};
  }catch(e){return {}}
}
async function roomFetch(code, opts={}){
  const URL="https://xbrrdkflztxkvnngmdhu.supabase.co";
  const KEY="sb_publishable_Ktw6Eh91X5K0yRjA9qJ6VA_vhxLPu8l";
  const headers=Object.assign({"apikey":KEY,"Authorization":`Bearer ${KEY}`,"Content-Type":"application/json"},opts.headers||{});
  return fetch(`${URL}/rest/v1/cronogol_rooms?room_code=eq.${encodeURIComponent(code)}`+(opts.query||""),Object.assign({},opts,{headers}));
}
async function pushHostRules(){
  try{
    const st=onlineStatus();
    const code=String(st.currentRoomCode||"").trim().toUpperCase();
    const role=String(st.currentRole||"").toLowerCase();
    if(!code || role!=="host") return;
    const mode=getRules() || (gameState && gameState.matchMode) || "";
    if(!mode) return;

    let state={};
    try{
      const r=await roomFetch(code,{query:"&select=state_json"});
      const rows=await r.json();
      state=rows && rows[0] && rows[0].state_json ? rows[0].state_json : {};
    }catch(e){}

    state.matchMode=mode;
    state.hostMatchMode=mode;
    state.rulesLockedByHost=true;
    state.rulesSyncedAt=new Date().toISOString();

    await roomFetch(code,{
      method:"PATCH",
      headers:{"Prefer":"return=minimal"},
      body:JSON.stringify({state_json:state,app_version:V,last_seen_at:new Date().toISOString()})
    });
  }catch(e){try{console.warn("[v2.5.7 pushHostRules]",e)}catch(_){}}
}
async function pullHostRules(){
  try{
    ...
    const r=await roomFetch(code,{query:"&select=state_json,status"});
    const rows=await r.json();
    const state=rows && rows[0] && rows[0].state_json ? rows[0].state_json : {};
    const hostMode=state.hostMatchMode || state.matchMode;

    if(hostMode && hostMode!==lastHostMode){
      lastHostMode=hostMode;
      if(gameState) gameState.matchMode=hostMode;
      const select=id("match-mode"); if(select) select.value=hostMode;
      qa('.segment-btn[data-target="match-mode"]').forEach(b=>{
        const on=b.dataset.value===hostMode;
  ...
  }catch(e){try{console.warn("[v2.5.7 pullHostRules]",e)}catch(_){}}
}

try{
  const prevApply=applyNormalResult;
  applyNormalResult=function(v,r){
    const mode=(gameState && gameState.matchMode) || getRules();
    if((mode==="five" || mode==="fast") && Number(v)%10===9){
      if(typeof startSpecial==="function") return startSpecial("penalty",v);
    }
    return prevApply.apply(this, arguments);
  };
}catch(e){}

setTimeout(clearDefaults,80);
...
/*
===============================================================================
CronoGol v2.5.7 — Rival Lock Final Fix
===============================================================================
...
/*
===============================================================================
CronoGol v2.5.7 — Online Clock & Match Complete
===============================================================================
Objetivo:
- Un único reloj online visible por ambos dispositivos.
- El jugador que tiene el turno publica su cronómetro oficial.
- El rival pinta el tiempo recibido, no su propio reloj local.
- El final online se publica en Supabase con status=finished.
- El rival recibe el final y queda bloqueado.
===============================================================================
*/
(function(){
"use strict";
...
let lastPulledClockId="";
let lastFinishId="";
let pushingClock=false;
let pulling=false;
let remoteFinalShown=false;
...
function onlineActive(){
  try{
    return Boolean(gameState && gameState.gameMode==="online" && gameScreen && gameScreen.classList.contains("active") && roomCode());
  }catch(e){return false;}
}
function isLocalTurn(){
  try{return onlineActive() && gameState.currentPlayerIndex===localIndex();}catch(e){return false;}
}
async function sf(path,opt={}){
  const headers=Object.assign({"apikey":KEY,"Authorization":`Bearer ${KEY}`,"Content-Type":"application/json"},opt.headers||{});
  const r=await fetch(`${URL}/rest/v1/${path}`,Object.assign({},opt,{headers}));
  const tx=await r.text();
  let data=null;
  if(tx){try{data=JSON.parse(tx)}catch(e){data=tx}}
  if(!r.ok){throw new Error(data&&data.message?data.message:`Supabase HTTP ${r.status}`)}
  return data;
}
function endpoint(code){return `${TABLE}?room_code=eq.${encodeURIComponent(code)}`;}
async function getRoomState(code){
  const rows=await sf(`${endpoint(code)}&select=state_json,status,updated_at`,{method:"GET"});
  if(!Array.isArray(rows)||!rows.length) return {state:{},status:""};
  return {state:rows[0].state_json||{},status:rows[0].status||""};
}
function playersSnapshot(){
  return (gameState.players||[]).map((p,i)=>({
    index:i,
    name:String(p&&p.name?p.name:`Jugador ${i+1}`).slice(0,24),
    goals:Number(p&&p.goals||0),
    skipTurns:Number(p&&p.skipTurns||0)
  }));
}
function scoreSnapshot(ps){
  ps=ps||playersSnapshot();
  return [Number(ps[0]&&ps[0].goals||0),Number(ps[1]&&ps[1].goals||0)];
}
function officialTimeText(ms){
  try{
...
function currentClockPayload(isRunningOverride){
  const now=Date.now();
  const elapsed=Math.max(0,Math.round(Number(currentElapsedMs||0)));
  return {
    schemaVersion:1,
    appVersion:V,
    clockId:`turn-${gameState.currentPlayerIndex}-${gameState.totalTurns}-${gameState.half}`,
    sourceRole:role()||"unknown",
    sourcePlayerIndex:Number(gameState.currentPlayerIndex||0),
    isRunning:Boolean(typeof isRunningOverride==="boolean"?isRunningOverride:gameState.isRunning),
    elapsedMs:elapsed,
    displayTime:officialTimeText(elapsed),
    lastTwo:Math.floor(elapsed/10)%100,
...
function finalPayload(reason){
  const ps=playersSnapshot();
  const score=scoreSnapshot(ps);
  let winnerIndex=null;
  if(score[0]>score[1]) winnerIndex=0;
  else if(score[1]>score[0]) winnerIndex=1;
  return {
    finishId:`finish-${Date.now()}-${Math.random().toString(36).slice(2,8)}`,
    schemaVersion:1,
    appVersion:V,
    reason:reason||"match_finished",
    status:"finished",
    phase:"finished",
    finalTime:timerDisplay?timerDisplay.textContent:officialTimeText(currentElapsedMs||0),
    finalElapsedMs:Math.max(0,Math.round(Number(currentElapsedMs||0))),
    finalScore:score,
    winnerIndex,
    winnerName:winnerIndex===null?null:ps[winnerIndex].name,
    players:ps,
    finishedAt:new Date().toISOString(),
    lastThrow:window.__cronogolLastThrowForAudit||null
  };
}
async function patchState(mutator,statusOverride){
  ...
  mutator(state);
  await sf(endpoint(code),{
    method:"PATCH",
    headers:{"Prefer":"return=minimal"},
    body:JSON.stringify({
      status:statusOverride || state.status || current.status || (gameState.matchEnded?"finished":"playing"),
      state_json:state,
      app_version:V,
      last_seen_at:new Date().toISOString()
    })
...
async function pushClock(reason,isRunningOverride){
  if(!onlineActive()||!isLocalTurn()||pushingClock||gameState.matchEnded) return;
  pushingClock=true;
  try{
    const clock=currentClockPayload(isRunningOverride);
    await patchState(state=>{
      ...
      state.matchMode=gameState.matchMode||state.matchMode||"classic";
      state.currentPlayerIndex=Number(gameState.currentPlayerIndex||0);
      state.half=Number(gameState.half||1);
      state.players=ps;
      state.score=scoreSnapshot(ps);
      state.stats=Object.assign({},gameState.stats||{});
      state.onlineClock=clock;
      state.lastClockReason=reason||"clock_tick";
      state.lastClockSyncAt=new Date().toISOString();
    },"playing");
  }catch(e){try{console.warn("[CronoGol v2.5 clock push]",e)}catch(_){}}
  finally{pushingClock=false;}
}
function applyRemoteClock(clock){
  if(!clock||!onlineActive()) return;
  // The local player owns their running clock; do not overwrite while they are actively timing.
  if(isLocalTurn() && gameState.isRunning) return;
  const id=String(clock.clockId||"");
  const elapsed=Number(clock.elapsedMs||0);
  ...
  try{ updateTimerDisplay(elapsed); }catch(e){
    ...
    // v2.5.7: no pisar lastTwoDisplay con onlineClock.lastTwo; la tirada real manda.
  }
  lastPulledClockId=id;
}
function applyRemoteFinal(finalState,state){
  if(!finalState||remoteFinalShown) return;
  if(lastFinishId && finalState.finishId===lastFinishId) return;
  lastFinishId=finalState.finishId||`remote-${Date.now()}`;
  remoteFinalShown=true;

  try{ if(gameState.isRunning) stopTimer(); }catch(e){}
  try{ clearInterval(timerInterval); }catch(e){}
  try{ clearInterval(matchClockInterval); }catch(e){}

  if(Array.isArray(finalState.players)&&finalState.players.length>=2){
    for(let i=0;i<2;i++){
      const p=finalState.players[i]||{};
      if(gameState.players&&gameState.players[i]){
        if(p.name) gameState.players[i].name=String(p.name).slice(0,24);
  ...
  if(state&&Number.isFinite(Number(state.currentPlayerIndex))){
    gameState.currentPlayerIndex=Number(state.currentPlayerIndex);
  }
  gameState.matchEnded=true;
  ...
  // v2.5.7: no pisar lastTwoDisplay con centésimas del reloj al finalizar.
  ...
  const score=finalState.finalScore?finalState.finalScore.join(" - "):scoreText();
  const text=finalState.winnerName?`${score}. Gana ${finalState.winnerName}.`:`${score}. Partido finalizado.`;
  try{
    showModal("FINAL DEL PARTIDO",text,finalHtml(),[
      {text:"NUEVA PARTIDA",action:resetToSetup}
    ]);
  }catch(e){}
}
async function pullOnlineState(){
  if(!onlineActive()||pulling) return;
  pulling=true;
  try{
    const row=await getRoomState(roomCode());
    const state=row.state||{};
    if(state.onlineClock) applyRemoteClock(state.onlineClock);
    if(row.status==="finished"||state.phase==="finished"||state.status==="finished"||state.finalState){
      applyRemoteFinal(state.finalState||state.finish||state, state);
    }
  }catch(e){try{console.warn("[CronoGol v2.5 pull]",e)}catch(_){}}
  finally{pulling=false;}
}
async function pushFinal(reason){
  if(!onlineActive()) return;
  const final=finalPayload(reason);
  lastFinishId=final.finishId;
  try{
    await patchState(state=>{
      const ps=playersSnapshot();
      state.schemaVersion=7;
      ...
      state.score=scoreSnapshot(ps);
      state.stats=Object.assign({},gameState.stats||{});
      state.onlineClock=currentClockPayload(false);
      state.finalState=final;
      state.finishedAt=final.finishedAt;
      state.matchEnded=true;
    },"finished");
  }catch(e){try{console.warn("[CronoGol v2.5 final push]",e)}catch(_){}}
}
function blockIfFinished(){
  if(!onlineActive()||!gameState.matchEnded) return;
  try{
    if(mainActionBtn){
      mainActionBtn.disabled=true;
      mainActionBtn.classList.add("start-disabled");
      mainActionBtn.textContent="FINAL";
    }
  }catch(e){}
}
try{
  const prevStart=startTimer;
  startTimer=function(){
    const out=prevStart.apply(this,arguments);
    if(onlineActive()&&isLocalTurn()) setTimeout(()=>pushClock("start",true),30);
    return out;
  };
}catch(e){}
try{
  const prevStop=stopTimer;
  stopTimer=function(){
    const out=prevStop.apply(this,arguments);
    if(onlineActive()&&isLocalTurn()) setTimeout(()=>pushClock("stop",false),30);
    return out;
  };
}catch(e){}
try{
  const prevUpdateTimerDisplay=updateTimerDisplay;
  updateTimerDisplay=function(ms){
    const out=prevUpdateTimerDisplay.apply(this,arguments);
    return out;
  };
}catch(e){}
try{
  const prevEnd=endMatch;
  endMatch=function(){
    const out=prevEnd.apply(this,arguments);
    if(onlineActive()) setTimeout(()=>pushFinal("endMatch"),120);
    return out;
  };
}catch(e){}
try{
  const prevShow=showFinal;
  showFinal=function(pens){
    const out=prevShow.apply(this,arguments);
    if(onlineActive()) setTimeout(()=>pushFinal(pens?"penalties":"showFinal"),120);
    return out;
  };
}catch(e){}
try{
  const prevSync=syncActionControls;
  syncActionControls=function(){
    const out=prevSync.apply(this,arguments);
    blockIfFinished();
    return out;
  };
}catch(e){}

// Safety: in quick/five mode, values ending in 9 are penalties before goal rules.
try{
  const prevEval=evaluateThrow;
  evaluateThrow=function(v){
    if(isFastMode && isFastMode() && Number(v)%10===9){
      return {type:"penalty",msg:"PENALTI",cls:"special",special:"penalty"};
    }
    return prevEval.apply(this,arguments);
  };
}catch(e){}

setInterval(()=>{
  if(!onlineActive()) return;
  // v2.5.7: STOP manda; no sincronizar reloj vivo. // pushClock("tick",true);
  pullOnlineState();
  blockIfFinished();
},700);

window.CronoGolOnlineClock=Object.freeze({version:V,pushClock,pullOnlineState,pushFinal});
})();



/* CronoGol v2.5.7 — Online Clock Display Fix */
(function(){
"use strict";
const V="2.5.7";
...
function st(){try{return window.CronoGolOnline&&window.CronoGolOnline.getOnlineStatus?window.CronoGolOnline.getOnlineStatus():{}}catch(e){return {}}}
function code(){return String(st().currentRoomCode||"").trim().toUpperCase()}
function protectThrow(t){
 if(!online()||!t)return;
 const txt=t.valueText||(Number.isFinite(Number(t.value))?pad(Number(t.value)):"");
 if(txt&&lastTwoDisplay)lastTwoDisplay.textContent=txt;
}
function remember(v){
 if(!online()||!Number.isFinite(Number(v)))return;
 const txt=pad(Number(v));
 if(lastTwoDisplay)lastTwoDisplay.textContent=txt;
 try{window.__cronogolLastThrowForAudit=Object.assign({},window.__cronogolLastThrowForAudit||{},{value:Number(v),valueText:txt})}catch(e){}
}
try{
 const prev=updateTimerDisplay;
 updateTimerDisplay=function(ms){
   const old=online()&&lastTwoDisplay?String(lastTwoDisplay.textContent||""):"";
   const out=prev.apply(this,arguments);
   if(online()&&/^\d{2}$/.test(old)&&old!=="--")lastTwoDisplay.textContent=old;
   return out;
 };
}catch(e){}
try{
 const prev=applyNormalResult;
 applyNormalResult=function(v,r){const out=prev.apply(this,arguments);setTimeout(()=>remember(v),20);setTimeout(()=>remember(v),180);return out}
}catch(e){}
try{
 const prev=evaluateSpecialThrow;
 evaluateSpecialThrow=function(v){const out=prev.apply(this,arguments);setTimeout(()=>remember(v),20);setTimeout(()=>remember(v),180);return out}
}catch(e){}
try{
 const prev=evaluateShootoutPenalty;
 evaluateShootoutPenalty=function(v){const out=prev.apply(this,arguments);setTimeout(()=>remember(v),20);setTimeout(()=>remember(v),180);return out}
}catch(e){}
async function sf(path,opt={}){
 const URL="https://xbrrdkflztxkvnngmdhu.supabase.co",KEY="sb_publishable_Ktw6Eh91X5K0yRjA9qJ6VA_vhxLPu8l";
 const headers=Object.assign({"apikey":KEY,"Authorization":`Bearer ${KEY}`,"Content-Type":"application/json"},opt.headers||{});
 const r=await fetch(`${URL}/rest/v1/${path}`,Object.assign({},opt,{headers}));
 const tx=await r.text();let d=null;if(tx){try{d=JSON.parse(tx)}catch(e){d=tx}}
 if(!r.ok)throw new Error(d&&d.message?d.message:`Supabase HTTP ${r.status}`);
...
async function pullProtect(){
 if(!online()||!code())return;
 try{
   const rows=await sf(`cronogol_rooms?room_code=eq.${encodeURIComponent(code())}&select=state_json,status`,{method:"GET"});
   if(!Array.isArray(rows)||!rows.length)return;
   const state=rows[0].state_json||{};
   if(state.lastThrow)protectThrow(state.lastThrow);
   if(state.finalState&&state.finalState.lastThrow)protectThrow(state.finalState.lastThrow);
 }catch(e){}
}
setInterval(pullProtect,900);
window.CronoGolClockDisplayFix=Object.freeze({version:V,protectThrow,pullProtect});
})();



/* CronoGol v2.5.7 — Last Throw Time Freeze Fix */
(function(){
"use strict";
const V="2.5.7";
...
function inferDisplayFromThrow(t){
 if(!t)return "";
 if(t.displayTime)return String(t.displayTime);
 if(Number.isFinite(Number(t.elapsedMs))){
   const ms=Number(t.elapsedMs), h=Math.floor(ms/10), m=Math.floor(h/6000), s=Math.floor((h%6000)/100), c=h%100;
   return `${pad2(m)}:${pad2(s)}:${pad2(c)}`;
 }
 const val=t.valueText||(Number.isFinite(Number(t.value))?pad2(Number(t.value)):"");
 return val?`00:00:${val}`:"";
}
function protectThrow(t){
 if(!online()||!t)return;
 const valueText=t.valueText||(Number.isFinite(Number(t.value))?pad2(Number(t.value)):"");
 const displayTime=inferDisplayFromThrow(t);
 protectedThrow=Object.assign({},t,{valueText,displayTime});
 if(valueText&&lastTwoDisplay)lastTwoDisplay.textContent=valueText;
 if(displayTime&&timerDisplay&&!gameState.isRunning)timerDisplay.textContent=displayTime;
}
function captureLocal(v){
 if(!online()||!Number.isFinite(Number(v)))return;
 const valueText=pad2(Number(v));
 const displayTime=timerDisplay?String(timerDisplay.textContent||""):`00:00:${valueText}`;
 protectedThrow={value:Number(v),valueText,displayTime,elapsedMs:(Number.isFinite(Number(currentElapsedMs))?Math.round(Number(currentElapsedMs)):null),id:`local-${Date.now()}`};
 try{window.__cronogolLastThrowForAudit=Object.assign({},window.__cronogolLastThrowForAudit||{},protectedThrow)}catch(e){}
 if(lastTwoDisplay)lastTwoDisplay.textContent=valueText;
 if(timerDisplay&&!gameState.isRunning)timerDisplay.textContent=displayTime;
}
function restoreIfWaiting(){
 if(!online()||!protectedThrow||gameState.isRunning)return;
 const currentRole=String(st().currentRole||"").toLowerCase();
 const localIdx=currentRole==="guest"?1:0;
 // If I'm waiting, or no clock is running, the board should show the last stopped throw,
 // not the next player's 00:00:01/02 pre-start clock.
 if(gameState.currentPlayerIndex!==localIdx || !gameState.isRunning){
   if(protectedThrow.displayTime&&timerDisplay)timerDisplay.textContent=protectedThrow.displayTime;
   if(protectedThrow.valueText&&lastTwoDisplay)lastTwoDisplay.textContent=protectedThrow.valueText;
 }
}
try{
 const prevUpdate=updateTimerDisplay;
 updateTimerDisplay=function(ms){
   const out=prevUpdate.apply(this,arguments);
   setTimeout(restoreIfWaiting,0);
   return out;
 };
}catch(e){}
try{
 const prevNormal=applyNormalResult;
 applyNormalResult=function(v,r){const out=prevNormal.apply(this,arguments);setTimeout(()=>captureLocal(v),0);setTimeout(()=>captureLocal(v),120);return out}
}catch(e){}
try{
 const prevSpecial=evaluateSpecialThrow;
 evaluateSpecialThrow=function(v){const out=prevSpecial.apply(this,arguments);setTimeout(()=>captureLocal(v),0);setTimeout(()=>captureLocal(v),120);return out}
}catch(e){}
try{
 const prevShoot=evaluateShootoutPenalty;
 evaluateShootoutPenalty=function(v){const out=prevShoot.apply(this,arguments);setTimeout(()=>captureLocal(v),0);setTimeout(()=>captureLocal(v),120);return out}
}catch(e){}
async function sf(path,opt={}){
 const URL="https://xbrrdkflztxkvnngmdhu.supabase.co",KEY="sb_publishable_Ktw6Eh91X5K0yRjA9qJ6VA_vhxLPu8l";
 const headers=Object.assign({"apikey":KEY,"Authorization":`Bearer ${KEY}`,"Content-Type":"application/json"},opt.headers||{});
...
const KEY="sb_publishable_Ktw6Eh91X5K0yRjA9qJ6VA_vhxLPu8l";
const TABLE="cronogol_rooms";
let protectedStop=null, lastAppliedStopId="", pushing=false, pulling=false;
...
function isLocalTurn(){try{return online()&&gameState.currentPlayerIndex===localIndex()}catch(e){return false}}
function fmt(ms){const h=Math.floor(Number(ms||0)/10),m=Math.floor(h/6000),s=Math.floor((h%6000)/100),c=h%100;return `${pad2(m)}:${pad2(s)}:${pad2(c)}`}
function score(){try{return [(gameState.players[0].goals||0),(gameState.players[1].goals||0)]}catch(e){return [0,0]}}
async function sf(path,opt={}){
 const headers=Object.assign({"apikey":KEY,"Authorization":`Bearer ${KEY}`,"Content-Type":"application/json"},opt.headers||{});
 const r=await fetch(`${URL}/rest/v1/${path}`,Object.assign({},opt,{headers}));
 const tx=await r.text();let d=null;if(tx){try{d=JSON.parse(tx)}catch(e){d=tx}}
...
function eventCls(){try{return Array.from(eventCard.classList||[]).find(c=>c.indexOf("event-")===0&&c!=="event-card")||"event-neutral"}catch(e){return "event-neutral"}}
function makeStop(v,kind){
 const elapsed=Number.isFinite(Number(currentElapsedMs))?Math.round(Number(currentElapsedMs)):0;
 const value=Number.isFinite(Number(v))?Number(v):(Math.floor(elapsed/10)%100);
 const valueText=pad2(value);
 const actorIndex=Number(gameState.currentPlayerIndex||0);
 const actor=gameState.players&&gameState.players[actorIndex]?gameState.players[actorIndex]:{name:`Jugador ${actorIndex+1}`};
 const title=eventTitle?String(eventTitle.textContent||""):"";
 const message=title&&title!=="--"?`${actor.name} sacó ${valueText} → ${title}`:(messageLabel?String(messageLabel.textContent||""):"");
 return {stopId:`stop-${Date.now()}-${Math.random().toString(36).slice(2,8)}`,appVersion:V,kind:kind||"normal",value,valueText,displayTime:fmt(elapsed),elapsedMs:elapsed,actorIndex,actorName:String(actor.name||"").slice(0,24),title,message,eventClass:eventCls(),score:score(),turnAfter:Number(gameState.currentPlayerIndex||0),createdAt:new Date().toISOString()};
}
function paint(stop){
 if(!online()||!stop)return;
 if(timerDisplay&&stop.displayTime)timerDisplay.textContent=stop.displayTime;
 if(lastTwoDisplay&&stop.valueText)lastTwoDisplay.textContent=stop.valueText;
...
async function pushStop(stop){
 if(!online()||!code()||!stop||pushing)return;
 pushing=true;
 try{
   const rows=await sf(`${endpoint()}&select=state_json,status`,{method:"GET"});
   const state=(Array.isArray(rows)&&rows[0]&&rows[0].state_json)?rows[0].state_json:{};
   state.schemaVersion=8; state.appVersion=V; state.phase=gameState.matchEnded?"finished":"playing"; state.status=state.phase; state.gameMode="online"; state.matchMode=gameState.matchMode||state.matchMode||"classic";
   state.currentPlayerIndex=Number(gameState.currentPlayerIndex||0);
   state.score=score();
   state.players=(gameState.players||[]).map((p,i)=>({index:i,name:String(p.name||`Jugador ${i+1}`).slice(0,24),goals:Number(p.goals||0),skipTurns:Number(p.skipTurns||0)}));
   state.lastStoppedThrow=stop;
 ...
 }catch(e){try{console.warn("[v2.5.7 pushStop]",e)}catch(_){}}
 finally{pushing=false}
}
function protect(v,kind){
 if(!online())return;
 const stop=makeStop(v,kind);
 protectedStop=stop;
 paint(stop);
 try{window.__cronogolLastThrowForAudit=Object.assign({},window.__cronogolLastThrowForAudit||{},stop)}catch(e){}
 setTimeout(()=>paint(stop),60);
 setTimeout(()=>pushStop(stop),120);
}
function applyRemote(stop){
 if(!online()||!stop)return;
 if(stop.stopId&&stop.stopId===lastAppliedStopId)return;
 lastAppliedStopId=stop.stopId||`remote-${Date.now()}`;
 protectedStop=stop;
 if(Array.isArray(stop.score)&&gameState.players&&gameState.players.length>=2){
   if(Number.isFinite(Number(stop.score[0])))gameState.players[0].goals=Number(stop.score[0]);
   if(Number.isFinite(Number(stop.score[1])))gameState.players[1].goals=Number(stop.score[1]);
 }
 try{updateUI()}catch(e){}
 paint(stop); setTimeout(()=>paint(stop),80);
}
function restore(){
 if(!online()||!protectedStop)return;
 if(!gameState.isRunning)paint(protectedStop);
}
async function pull(){
 ...
 try{
   ...
   restore();
 }catch(e){try{console.warn("[v2.5.7 pull]",e)}catch(_){}}
 finally{pulling=false}
}
...
try{const prev=evaluateSpecialThrow;evaluateSpecialThrow=function(v){const out=prev.apply(this,arguments);setTimeout(()=>protect(v,"special"),0);setTimeout(()=>protect(v,"special"),150);return out}}catch(e){}
...
function renderMs(ms){
  if(!online()||!gameState.isRunning||!isLocalTurn())return;
  const h=Math.floor(Number(ms||0)/10);
  const m=Math.floor(h/6000);
  const s=Math.floor((h%6000)/100);
  const c=h%100;
  if(timerDisplay)timerDisplay.textContent=`${p2(m)}:${p2(s)}:${p2(c)}`;
  if(lastTwoDisplay)lastTwoDisplay.textContent=p2(c);
}

function forceLiveVisual(){
  if(!online()||!isLocalTurn()||!gameState.isRunning)return;
  try{renderMs(currentElapsedMs)}catch(e){}
}
...
try{
  ...
  startTimer=function(){
    const out=prevStart.apply(this,arguments);
    if(online()&&isLocalTurn()){
      setTimeout(forceLiveVisual,0);
      setTimeout(forceLiveVisual,25);
      setTimeout(forceLiveVisual,80);
      setTimeout(forceLiveVisual,160);
      setTimeout(forceLiveVisual,320);
    }
    return out;
  };
}catch(e){}
...
try{
  ...
  updateTimerDisplay=function(ms){
    const out=prevUpdate.apply(this,arguments);
    if(online()&&isLocalTurn()&&gameState.isRunning){
      setTimeout(()=>renderMs(ms),0);
      setTimeout(()=>renderMs(ms),15);
    }
    return out;
  };
}catch(e){}
...
try{
  ...
  evaluateThrow=function(v){
    const n=Number(v);
    const mode=(gameState&&gameState.matchMode)||"";
    if((mode==="five"||mode==="fast")&&Number.isFinite(n)){
      if(n===1||n===2)return {type:"post",msg:"POSTE",cls:"post",repeat:true};
      if(n===3||n===4)return {type:"crossbar",msg:"LARGUERO",cls:"crossbar",repeat:true};
      if(n===50)return {type:"yellow",msg:"AMARILLA",cls:"yellow",card:"yellow"};
      if(n===60)return {type:"red",msg:"ROJA",cls:"red",card:"red"};
      if(n===96||n===97)return {type:"free",msg:"FALTA PELIGROSA",cls:"special",special:"free"};
      if(n%10===9)return {type:"penalty",msg:"PENALTI",cls:"special",special:"penalty"};
      if(n%10===0)return {type:"goal",msg:"GOL",cls:"goal"};
      return {type:"miss",msg:"FALLO",cls:"miss"};
    }
    return previousEvaluateThrow.apply(this,arguments);
  };
}catch(e){}

window.CronoGolStartVisualFastFix=Object.freeze({version:V,forceLiveVisual});
})();



/* CronoGol v2.5.7 — Remote Stop Paint & Timer Smoothing */
(function(){
"use strict";
const V="2.5.7",URL="https://xbrrdkflztxkvnngmdhu.supabase.co",KEY="sb_publishable_Ktw6Eh91X5K0yRjA9qJ6VA_vhxLPu8l",TABLE="cronogol_rooms";
...
try{const pu=updateTimerDisplay;updateTimerDisplay=function(ms){const out=pu.apply(this,arguments);if(online()&&isLocalTurn()&&gameState.isRunning)renderLocal(ms);return out}}catch(e){}
function afterStop(){pollingFastUntil=Date.now()+3000;setTimeout(pull,100)}
...
setInterval(()=>{if(Date.now()<pollingFastUntil)pull()},250);setInterval(pull,900);
...
/*
===============================================================================
CronoGol v2.5.7 — Monotonic Timer Render Guard
===============================================================================
Problema corregido:
- Mientras el cronómetro corre, algunas capas antiguas podían repintar un valor menor:
  ejemplo real detectado en vídeo: 00:10:73 -> 00:09:75.
Regla nueva:
- Si el cronómetro está corriendo, el display grande solo puede avanzar o quedarse igual.
- El STOP oficial sigue pudiendo congelar el valor final exacto.
- En espera se sigue permitiendo pintar el último STOP oficial remoto/local.
===============================================================================
*/
(function(){
"use strict";
...
function online(){
  try{return gameState&&gameState.gameMode==="online"&&gameScreen&&gameScreen.classList.contains("active")}catch(e){return false}
}
function st(){
...
function localIndex(){return role()==="guest"?1:0}
function isLocalTurn(){
  try{return online()&&gameState.currentPlayerIndex===localIndex()}catch(e){return false}
}
function p2(n){return String(n).padStart(2,"0")}
function fmt(ms){
  const h=Math.floor(Number(ms||0)/10);
...
function parseDisplayMs(text){
  const m=String(text||"").match(/(\d{2}):(\d{2}):(\d{2})/);
  if(!m)return null;
  const min=Number(m[1]), sec=Number(m[2]), cen=Number(m[3]);
  if(!Number.isFinite(min)||!Number.isFinite(sec)||!Number.isFinite(cen))return null;
  return ((min*60)+sec)*1000 + cen*10;
}
function ownerKey(){
  try{return `${role()||"local"}:${Number(gameState.currentPlayerIndex||0)}:${gameState.half||1}:${gameState.matchMode||""}`}catch(e){return "local"}
}
function resetGuard(reason){
  runningGuard.active=false;
  runningGuard.ownerKey=ownerKey();
  runningGuard.lastMs=0;
  runningGuard.lastText="00:00:00";
  runningGuard.resetAt=Date.now();
  try{window.__cronogolMonotonicGuardLastReset={reason,at:new Date().toISOString()}}catch(e){}
}
function beginGuard(reason){
  runningGuard.active=true;
  runningGuard.ownerKey=ownerKey();
  const current = parseDisplayMs(timerDisplay && timerDisplay.textContent);
  const elapsed = Number.isFinite(Number(currentElapsedMs)) ? Number(currentElapsedMs) : null;
  runningGuard.lastMs = Math.max(0, elapsed!==null ? elapsed : (current!==null ? current : 0));
  runningGuard.lastText = fmt(runningGuard.lastMs);
  try{window.__cronogolMonotonicGuard={version:V,reason,active:true,lastMs:runningGuard.lastMs,lastText:runningGuard.lastText}}catch(e){}
}
function shouldGuard(){
  return online() && gameState && gameState.isRunning && isLocalTurn();
}
function monotonicMs(ms){
  const n = Math.max(0, Number(ms||0));
  if(!shouldGuard())return n;
  const key = ownerKey();
  if(!runningGuard.active || runningGuard.ownerKey!==key)beginGuard("auto");
  if(n < runningGuard.lastMs){
    try{window.__cronogolBlockedTimerRetrograde={from:runningGuard.lastText,to:fmt(n),fromMs:runningGuard.lastMs,toMs:n,at:new Date().toISOString()}}catch(e){}
    return runningGuard.lastMs;
  }
  runningGuard.lastMs=n;
  runningGuard.lastText=fmt(n);
  return n;
}
function paintMonotonic(ms){
  if(!shouldGuard())return false;
  const safeMs=monotonicMs(ms);
  const text=fmt(safeMs);
...
try{
  ...
  startTimer=function(){
    ...
    if(online()&&isLocalTurn()){
      beginGuard("start");
      setTimeout(()=>paintMonotonic(currentElapsedMs),0);
      setTimeout(()=>paintMonotonic(currentElapsedMs),30);
      setTimeout(()=>paintMonotonic(currentElapsedMs),90);
    }
    return out;
  };
}catch(e){}
...
try{
  const prevStop=stopTimer;
  stopTimer=function(){
    // Antes de parar, pintamos una última vez monotónico; después STOP oficial puede congelar.
    if(shouldGuard())paintMonotonic(currentElapsedMs);
    const out=prevStop.apply(this,arguments);
    resetGuard("stop");
    return out;
  };
...
try{
  ...
  updateTimerDisplay=function(ms){
    ...
    return prevUpdate.apply(this,arguments);
  };
}catch(e){}

setInterval(()=>{
  if(shouldGuard()){
    try{paintMonotonic(currentElapsedMs)}catch(e){}
  }
},50);

// Reset claro cuando se procesa un resultado y cambia el estado de carrera/turno.
function afterOfficialStop(){
  setTimeout(()=>resetGuard("official-stop"),40);
}
try{
  const prevNormal=applyNormalResult;
  applyNormalResult=function(v,r){
    const out=prevNormal.apply(this,arguments);
    afterOfficialStop();
    return out;
  };
}catch(e){}
try{
  const prevSpecial=evaluateSpecialThrow;
  evaluateSpecialThrow=function(v){
    const out=prevSpecial.apply(this,arguments);
    afterOfficialStop();
    return out;
  };
}catch(e){}
try{
  const prevShoot=evaluateShootoutPenalty;
  evaluateShootoutPenalty=function(v){
    const out=prevShoot.apply(this,arguments);
    afterOfficialStop();
    return out;
  };
}catch(e){}

window.CronoGolMonotonicTimerGuard=Object.freeze({
  version:V,
  parseDisplayMs,
  monotonicMs,
  paintMonotonic,
...
- Se podían mezclar estados visuales: reloj grande vivo de otra capa + número/evento de STOP.
  Ejemplo detectado: reloj 00:11:43, número 70, evento GOL.
Regla final:
- El display grande tiene un único dueño según estado.
- RUNNING_LOCAL: pinta reloj vivo local.
- STOP_FROZEN / WAITING_REMOTE / SPECIAL_PENDING / FINISHED: pinta último STOP oficial.
- Rival corriendo: no se enseña el reloj vivo del rival; se mantiene el último STOP.
===============================================================================
*/
(function(){
"use strict";
...
function normStop(raw){
 if(!raw)return null;
 const v=Number.isFinite(Number(raw.value))?Number(raw.value):null;
 const vt=raw.valueText||(v!==null?p2(v):"");
 const ms=Number.isFinite(Number(raw.elapsedMs))?Number(raw.elapsedMs):null;
 const dt=raw.displayTime||(ms!==null?fmt(ms):(vt?`00:00:${vt}`:""));
 return Object.assign({},raw,{stopId:raw.stopId||raw.id||`stop-${raw.createdAt||Date.now()}-${vt}`,value:v,valueText:vt,elapsedMs:ms,displayTime:dt,title:raw.title||"",message:raw.message||"",eventClass:raw.eventClass||"event-neutral"});
}
function setOwner(next, reason){
 if(owner!==next){ ownerSeq++; }
 owner=next;
 try{window.__cronogolDisplayOwner={version:V,owner,reason,seq:ownerSeq,at:new Date().toISOString()}}catch(e){}
}
function eventCls(){
 try{return Array.from(eventCard.classList||[]).find(c=>c.indexOf("event-")===0&&c!=="event-card")||"event-neutral"}catch(e){return "event-neutral"}
}
function score(){
 try{return [(gameState.players[0].goals||0),(gameState.players[1].goals||0)]}catch(e){return [0,0]}
}
function makeLocalStop(v,kind){
 const elapsed=Number.isFinite(Number(currentElapsedMs))?Math.round(Number(currentElapsedMs)):0;
 const value=Number.isFinite(Number(v))?Number(v):(Math.floor(elapsed/10)%100);
 const valueText=p2(value);
 const actorIndex=Number(gameState.currentPlayerIndex||0);
 const actor=gameState.players&&gameState.players[actorIndex]?gameState.players[actorIndex]:{name:`Jugador ${actorIndex+1}`};
 const title=eventTitle?String(eventTitle.textContent||""):"";
 const msg=title&&title!=="--"?`${actor.name} sacó ${valueText} → ${title}`:(messageLabel?String(messageLabel.textContent||""):"");
 return {stopId:`own-${Date.now()}-${Math.random().toString(36).slice(2,8)}`,kind:kind||"normal",value,valueText,elapsedMs:elapsed,displayTime:fmt(elapsed),actorIndex,actorName:String(actor.name||"").slice(0,24),title,message:msg,eventClass:eventCls(),score:score(),turnAfter:Number(gameState.currentPlayerIndex||0),createdAt:new Date().toISOString(),appVersion:V};
}
function applyStop(stop, reason){
 stop=normStop(stop); if(!online()||!stop)return;
 frozenStop=stop; lastFrozenStopId=stop.stopId;
 if(Array.isArray(stop.score)&&gameState.players&&gameState.players.length>=2){
   if(Number.isFinite(Number(stop.score[0])))gameState.players[0].goals=Number(stop.score[0]);
   if(Number.isFinite(Number(stop.score[1])))gameState.players[1].goals=Number(stop.score[1]);
 }
 if(Number.isFinite(Number(stop.turnAfter))) gameState.currentPlayerIndex=Number(stop.turnAfter);
 if(stop.title&&/PENALTI|FALTA/i.test(stop.title)) setOwner("SPECIAL_PENDING", reason||"special");
 else if(gameState.matchEnded) setOwner("FINISHED", reason||"finished");
 else setOwner(isLocalTurn()?"STOP_FROZEN":"WAITING_REMOTE", reason||"stop");
 paintOwned();
 setTimeout(paintOwned,50);
 setTimeout(paintOwned,160);
}
function paintFrozen(){
 if(!frozenStop)return false;
 if(timerDisplay&&frozenStop.displayTime)timerDisplay.textContent=frozenStop.displayTime;
 if(lastTwoDisplay&&frozenStop.valueText)lastTwoDisplay.textContent=frozenStop.valueText;
 if(eventTitle&&frozenStop.title)eventTitle.textContent=frozenStop.title;
 if(messageLabel&&frozenStop.message)messageLabel.textContent=frozenStop.message;
 if(eventCard&&frozenStop.eventClass)eventCard.className=`event-card ${frozenStop.eventClass}`;
 return true;
}
function paintRunning(ms){
 const text=fmt(ms);
 if(timerDisplay)timerDisplay.textContent=text;
 if(lastTwoDisplay)lastTwoDisplay.textContent=text.slice(-2);
 return true;
...
function paintOwned(){
 ...
 if(gameState.isRunning && isLocalTurn()){
   setOwner("RUNNING_LOCAL","paint");
   return paintRunning(Number.isFinite(Number(currentElapsedMs))?Number(currentElapsedMs):0);
 }
 // If rival is running or we are waiting, frozen STOP owns display.
 if(frozenStop){
   if(gameState.matchEnded) setOwner("FINISHED","paint");
   else if(owner==="SPECIAL_PENDING") setOwner("SPECIAL_PENDING","paint");
   else setOwner(isLocalTurn()?"STOP_FROZEN":"WAITING_REMOTE","paint");
   return paintFrozen();
 }
 return false;
}
async function pushLocalStop(stop){
 if(!online()||!code()||!stop)return;
 try{
   const rows=await sf(`${endpoint()}&select=state_json,status`,{method:"GET"});
   const state=(Array.isArray(rows)&&rows[0]&&rows[0].state_json)?rows[0].state_json:{};
   state.schemaVersion=9; state.appVersion=V; state.phase=gameState.matchEnded?"finished":"playing"; state.status=state.phase; state.gameMode="online"; state.matchMode=gameState.matchMode||state.matchMode||"classic";
   state.currentPlayerIndex=Number(gameState.currentPlayerIndex||0);
   state.score=score();
   state.players=(gameState.players||[]).map((p,i)=>({index:i,name:String(p.name||`Jugador ${i+1}`).slice(0,24),goals:Number(p.goals||0),skipTurns:Number(p.skipTurns||0)}));
   state.lastStoppedThrow=stop;
   ...
   state.onlineClock={mode:"stopped",isRunning:false,elapsedMs:stop.elapsedMs,displayTime:stop.displayTime,lastTwo:stop.value,sourcePlayerIndex:stop.actorIndex,updatedAt:stop.createdAt};
   await sf(endpoint(),{method:"PATCH",headers:{"Prefer":"return=minimal"},body:JSON.stringify({status:state.status,state_json:state,app_version:V,last_seen_at:new Date().toISOString()})});
 }catch(e){try{console.warn("[v2.5.7 pushLocalStop]",e)}catch(_){}}
}
async function pullStop(){
 if(!online()||!code()||pulling)return;
 // During my live run, display is owned locally. We can skip remote paint.
 if(gameState.isRunning&&isLocalTurn())return;
 pulling=true;
 try{
   const rows=await sf(`${endpoint()}&select=state_json,status,updated_at`,{method:"GET"});
   if(!Array.isArray(rows)||!rows.length)return;
   const state=rows[0].state_json||{};
   const stop=state.lastStoppedThrow||(state.lastThrow&&state.lastThrow.displayTime?Object.assign({stopId:state.lastThrow.id},state.lastThrow):null);
   const ns=normStop(stop);
   if(ns && ns.stopId!==lastFrozenStopId){
      applyStop(ns,"remote-new-stop");
   }else{
      paintOwned();
   }
   if((rows[0].status==="finished"||state.phase==="finished"||state.status==="finished")&&state.finalState&&state.finalState.lastThrow){
      const fs=normStop(state.finalState.lastThrow);
      if(fs && fs.stopId!==lastFrozenStopId)applyStop(fs,"final");
   }
 }catch(e){try{console.warn("[v2.5.7 pullStop]",e)}catch(_){}}
 finally{pulling=false}
}
function afterLocalStop(v,kind){
 if(!online())return;
 const stop=makeLocalStop(v,kind);
 applyStop(stop,"local-stop");
 setTimeout(()=>pushLocalStop(stop),80);
}
try{
 const prevStart=startTimer;
 startTimer=function(){
   const out=prevStart.apply(this,arguments);
   if(online()&&isLocalTurn()){
      setOwner("RUNNING_LOCAL","start");
      setTimeout(paintOwned,0); setTimeout(paintOwned,30); setTimeout(paintOwned,100);
   }
   return out;
 };
}catch(e){}
try{
 const prevStop=stopTimer;
 stopTimer=function(){
   if(online()&&isLocalTurn())paintOwned();
   const out=prevStop.apply(this,arguments);
   return out;
 };
}catch(e){}
try{
 ...
 updateTimerDisplay=function(ms){
   if(online()){
      if(gameState.isRunning&&isLocalTurn()){
        setOwner("RUNNING_LOCAL","update");
        return paintRunning(ms);
      }
      return paintOwned() || prevUpdate.apply(this,arguments);
   }
   return prevUpdate.apply(this,arguments);
 };
}catch(e){}
try{
 const p=applyNormalResult;
 applyNormalResult=function(v,r){const out=p.apply(this,arguments);setTimeout(()=>afterLocalStop(v,"normal"),0);setTimeout(paintOwned,120);return out}
}catch(e){}
try{
 const p=evaluateSpecialThrow;
 evaluateSpecialThrow=function(v){const out=p.apply(this,arguments);setTimeout(()=>afterLocalStop(v,"special"),0);setTimeout(paintOwned,120);return out}
}catch(e){}
try{
 const p=evaluateShootoutPenalty;
 evaluateShootoutPenalty=function(v){const out=p.apply(this,arguments);setTimeout(()=>afterLocalStop(v,"shootout"),0);setTimeout(paintOwned,120);return out}
}catch(e){}
// Blindaje final de modo rápido: terminados en 0 son gol salvo 50/60.
try{
 const previousEvaluateThrow=evaluateThrow;
 evaluateThrow=function(v){
   const n=Number(v), mode=(gameState&&gameState.matchMode)||"";
   if((mode==="five"||mode==="fast")&&Number.isFinite(n)){
     if(n===1||n===2)return {type:"post",msg:"POSTE",cls:"post",repeat:true};
     if(n===3||n===4)return {type:"crossbar",msg:"LARGUERO",cls:"crossbar",repeat:true};
     if(n===50)return {type:"yellow",msg:"AMARILLA",cls:"yellow",card:"yellow"};
...
setInterval(()=>{paintOwned()},80);
setInterval(()=>{pullStop()},500);
window.CronoGolDisplayOwnership=Object.freeze({version:V,setOwner,paintOwned,applyStop,pullStop,get owner(){return owner},get frozenStop(){return frozenStop}});
})();
