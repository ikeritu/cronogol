...
(function(){
  ...
  function setStatus(message,type){const status=document.getElementById("cg-online-status"); if(status){status.textContent=message; status.dataset.state=type||"neutral"; const box=status.closest(".online-lobby-safe-status"); if(box) box.dataset.state=type||"neutral";}}
  ...
  function setLobby(room){const host=document.getElementById("cg-online-host-name"); const guest=document.getElementById("cg-online-guest-name"); const state=document.getElementById("cg-online-room-state"); const hostBadge=document.getElementById("cg-online-host-badge"); const guestBadge=document.getElementById("cg-online-guest-badge"); if(!room){if(host) host.textContent="—"; if(guest) guest.textContent="—"; if(state) state.textContent="Sin sala"; if(hostBadge){hostBadge.textContent="Conectado"; hostBadge.className="online-lobby-safe-pill connected";} if(guestBadge){guestBadge.textContent="Esperando rival"; guestBadge.className="online-lobby-safe-pill waiting";} return;} if(host) host.textContent=room.hostName||"Jugador 1"; if(guest) guest.textContent=room.guestName||"—"; if(hostBadge){hostBadge.textContent="Conectado"; hostBadge.className="online-lobby-safe-pill connected";} if(state){if(room.status==="ready"&&room.guestName) state.textContent="Rival conectado"; else if(room.status==="playing") state.textContent="Partida en curso"; else state.textContent="Esperando rival";} if(guestBadge){if(room.status==="ready"&&room.guestName){guestBadge.textContent="Conectado"; guestBadge.className="online-lobby-safe-pill connected";} else if(room.status==="playing"&&room.guestName){guestBadge.textContent="En partida"; guestBadge.className="online-lobby-safe-pill connected";} else {guestBadge.textContent="Esperando rival"; guestBadge.className="online-lobby-safe-pill waiting";}}}
